...
function buildRouting(options) {
    ...
    if (!handler && typeof options === 'function') {
      ...
      if (Object.prototype.toString.call(options) !== '[object Object]') {
        ...
        if (typeof options.handler === 'function') {
          throw new Error(
            `Duplicate handler for ${method}:${url} route is not allowed!`
          )
        } else {
          throw new Error(
            `Handler for ${method}:${url} route must be a function`
          )
        }
      }
    }
  ...
  function route(options) {
    ...
    throwIfAlreadyStarted(
      'Cannot add route when fastify instance is already started!'
    )
    ...
    if (!opts.handler) {
      throw new Error(
        `Missing handler function for ${opts.method}:${path} route.`
      )
    }
    ...
    function afterRouteAdded({ path, prefixing = false }, notHandledErr, done) {
      ...
      if (shouldExposeHead && options.method === 'GET' && !headRouteExists) {
        ...
        prepareRoute.call(this, 'HEAD', path, {
          ...opts,
        ...
        for (const hook of lifecycleHooks) {
          const toSet = this[kHooks][hook].concat(opts[hook] || []).map((h) => {
            ...
            if (hook === 'preParsing') {
              ...
              if (
                h.length === (h.constructor.name === 'AsyncFunction' ? 2 : 3)
              ) {
  ...
  function routeHandler(req, res, params, context) {
    ...
    const query = querystringParser(
      queryPrefix > -1 ? req.url.slice(queryPrefix + 1) : ''
    )
...
function validateBodyLimitOption(bodyLimit) {
  ...
  if (!Number.isInteger(bodyLimit) || bodyLimit <= 0) {
    throw new TypeError(
      `'bodyLimit' option must be an integer > 0. Got '${bodyLimit}'`
    )
  }
}
