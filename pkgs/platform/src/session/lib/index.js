...
function decryptSession(sessionId, options, request, done) {
  ...
  if (decryptedSessionId === false) {
    ...
    const key = decryptedSessionId.split('.').shift()
    options.store.get(key, (err, session) => {
      if (err) {
        if (err.code === 'ENOENT') {
          newSession(secret, request, cookieOpts, idGenerator, done)
        } else {
      ...
      if (session && session.expires && session.expires <= Date.now()) {
        options.store.destroy(
          sessionId,
          getDestroyCallback(secret, request, done, cookieOpts, idGenerator)
        )
        return
      }
      request.session = new Session(idGenerator, cookieOpts, secret, session)
      done()
    })
  }
}
  ...
  return function handleSession(request, reply, done) {
    request.handleSession = () => {
      return new Promise((resolve) => {
        ...
        if (!sessionId) {
          newSession(secret, request, cookieOpts, idGenerator, resolve)
        } else {
          decryptSession(sessionId, options, request, resolve)
        }
      })
    }
    done()
...
function onSend(options) {
  return function saveSession(request, reply, payload, done) {
    ...
    if (
      !session ||
      !session.sessionId ||
      !shouldSaveSession(request, options.cookie, options.saveUninitialized)
    ) {
      done()
      return
    }
...
export default fastifyPlugin.default(session, metadata)
