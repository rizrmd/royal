...
export async function findFreePorts(
  ...
  if (count > endPort - startPort) {
    throw new Error(
      `Could not find free ports: the range of allowed ports is not large enough for the requested amount of ports to find.`
    )
  }
  ...
  for (let i = 0; i < jobCount; i++) {
    jobPromises.push(
      scanRange(
        startPort + portInterval * i,
        Math.min(endPort, startPort + portInterval * (i + 1))
      )
    )
  }
